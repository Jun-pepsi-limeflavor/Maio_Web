...
export async function POST(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    return Response.json({ success: false, message: '라벨 저장 중 오류가 발생했습니다.' });
  }
}
...
export async function uploadHandler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!file) {
      return Response.json({ success: false, message: '파일이 없습니다.' });
    }
    ...
    console.error('Error:', error);
    return Response.json({ success: false, message: '파일 업로드 중 오류가 발생했습니다.' });
  }
}
...
export async function initialize(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const data = await response.json();
    return Response.json({ client_id: data.client_id });
  } catch (error) {
    console.error('Error:', error);
    return Response.json({ error: '초기화 중 오류가 발생했습니다.' });
  }
}